...
# Ensure local.properties exists and update only the necessary keys
...
# Helper to set or append a key=value in a properties file
set_prop() {
...
# SDK location
set_prop sdk.dir                "$ANDROID_SDK_ROOT"     "$LOCAL_PROP"
# App environment variables
set_prop kakao.native.app.key   "$KAKAO_NATIVE_APP_KEY" "$LOCAL_PROP"
set_prop server.base.url        "$SERVER_BASE_URL"      "$LOCAL_PROP"

# If you also want the script to manage keystore settings automatically,
# uncomment and set the following (ensure KEYSTORE_PASSWORD and KEYSTORE_ALIAS are exported):
# set_prop keystore.file.path    "app/keystore/release.keystore" "$LOCAL_PROP"
# set_prop keystore.password     "$KEYSTORE_PASSWORD"            "$LOCAL_PROP"
# set_prop keystore.key.alias    "$KEYSTORE_ALIAS"               "$LOCAL_PROP"
# set_prop keystore.key.password "$KEYSTORE_PASSWORD"            "$LOCAL_PROP"
