...
def determine_query_intent(query: str) -> str:
    ...
    # 사진 검색 키워드 확장
    photo_keywords = [
        "사진", "찾아", "보여", "이미지", "찾아줘", "있나", "있어", "어디",
        "찍은", "촬영", "찰칵", "어딨", "보고싶", "볼래"
    ]
    
    # 색상 키워드 (사진 검색의 강력한 지표)
    color_keywords = [
        "빨간", "파란", "노란", "초록", "검은", "하얀", "분홍", "보라", "주황",
        "회색", "갈색", "금색", "은색", "하늘색", "연두색"
    ]
    
    # 객체/물건 키워드 (사진에 자주 등장하는 대상) - 에어컨 추가
    object_keywords = [
        "인형", "장난감", "물건", "옷", "차", "집", "동물", "꽃", "나무",
        "가방", "신발", "모자", "시계", "책", "음식", "케이크", "선물",
        "에어컨", "에어컨디셔너", "냉방기", "냉방"
    ]
    
    # 정보 요청 키워드 (명확히 정보를 원하는 경우)
    info_keywords = ["설명", "무엇", "뜻", "의미", "정의", "역사", "유래", "방법", "이유", "어떻게"]
...
def enhance_query_with_personal_context_v2(user_id: str, query: str) -> list[str]:
    ...
    if not found_objects:
        for word in query_words:
            if len(word) >= 2 and word not in ["사진", "찾아", "보여", "있나", "있어", "줘", "주세요"]:
...
def filter_relevant_items_with_context(
    ...
    print(f"🎯 필터링 시작: {item_type}, 입력 항목 수: {len(items)}, 쿼리: {original_query}")
    ...
    objects = ["인형", "에어컨", "드레스", "차", "꽃", "케이크", "가방", "장난감", "옷", "동물"]
    found_color = None
    found_object = None
    ...
    if not found_object:
        ...
        for word in words:
            if word not in colors and len(word) >= 2 and word not in ["사진", "찾아", "보여", "있나"]:
                found_object = word
                break
    ...
    # 조정된 점수로 정렬
    filtered_items.sort(key=lambda x: x.get("adjusted_score", x.get("score", 0)), reverse=True)
