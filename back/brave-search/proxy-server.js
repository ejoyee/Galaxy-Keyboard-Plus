...
function startMCPProcess() {
  childProcess = spawn('npx', ['@modelcontextprotocol/server-brave-search', '--foreground']);
      ...
      const responseHandler = (data) => {
        ...
        try {
          ...
          if (validJsonFound && response.id === requestId) {
            ...
            console.log('Received response for different request, still waiting');
            responseBuffer = '';
          }
        } catch (e) {
...
// API 키 상태 확인
console.log(`BRAVE_API_KEY is ${process.env.BRAVE_API_KEY ? 'set' : 'NOT set'}`);
