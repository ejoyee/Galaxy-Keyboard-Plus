...
function startMCPProcess() {
  childProcess = spawn(
    "npx",
    ["@modelcontextprotocol/server-brave-search", "--foreground"],
    {
      cwd: "/app",
      env: {
        ...process.env,
  ...
  childProcess.on("close", (code) => {
    ...
    if (code !== 0 && !isShuttingDown) {
      console.log("[Brave MCP] 프로세스 비정상 종료, 5초 후 재시작 시도...");
...
app.post("/", async (req, res) => {
  try {
    ...
    console.log("[Brave MCP] Received HTTP request:", JSON.stringify(request));
    ...
    console.log("[Brave MCP] Transformed request:", JSON.stringify(mcpRequest));
      ...
      const responseHandler = (data) => {
        ...
        try {
          ...
          if (validJsonFound && response.id === requestId) {
            ...
            removeResponseHandler();
            console.log(
              "[Brave MCP] Received MCP response:",
            ...
            console.log(
              "[Brave MCP] Received response for different request, still waiting"
            );
            responseBuffer = "";
          }
        } catch (e) {
          console.error("[Brave MCP] Error parsing response:", e);
        }
      };
    ...
    console.log("[Brave MCP] Sending HTTP response:", JSON.stringify(response));
    res.json(response);
  } catch (error) {
    console.error("[Brave MCP] Error processing request:", error);
    res.status(500).json({
      jsonrpc: "2.0",
      id: req.body.id || null,
      error: {
        code: -32000,
        message: `[Brave MCP] Internal server error: ${error.message}`,
      },
    });
  }
});
