    ...
    switch (method) {
      ...
      case 'read_files': {
        if (!params.path || !params.ext)
          throw new Error('Missing param: path or ext');
        const result = readFilesWithExtension(params.path, params.ext);
        console.log(`read_files returned ${result.length} files`);
