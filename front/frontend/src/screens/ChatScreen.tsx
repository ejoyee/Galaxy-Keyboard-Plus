...
export default function ChatScreen() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (typeof apiResponse !== 'object' || apiResponse === null || !apiResponse.query_type) {
        console.error("잘못된 API 응답 형식:", apiResponse);
        throw new Error("서버로부터 유효한 응답을 받지 못했습니다.");
      }

      if (apiResponse.query_type === "info" || apiResponse.query_type === "ambiguous") {
      ...
      if (axios.isAxiosError(e)) {
        ...
        console.error("Axios error details:", e.response?.data, e.response?.status, e.config);
        setError(e.response?.data?.message || e.message || "서버 통신 중 오류가 발생했습니다.");
      } else {
        setError(e.message || "검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
      }
      // 선택적으로 채팅에 오류 메시지 추가
    ...
    return (
      <View style={[styles.row, isUser ? styles.rowUser : styles.rowBot]}>
        ...
        >
          ...
          {/* 사진 검색 결과 (봇 전용) */}
          {item.sender === "bot" && item.photo_results && item.photo_results.length > 0 && (
            <View style={styles.grid}>
              {item.photo_results.map((photo) => (
                <View key={photo.id} style={styles.gridItem}>
                  <Text style={styles.gridItemText} numberOfLines={3}>{photo.text}</Text>
                  <Text style={styles.resultScoreText}>유사도: {photo.score.toFixed(2)}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {/* 정보 검색 결과 (봇 전용) */}
          {item.sender === "bot" && item.info_results && item.info_results.length > 0 && (
