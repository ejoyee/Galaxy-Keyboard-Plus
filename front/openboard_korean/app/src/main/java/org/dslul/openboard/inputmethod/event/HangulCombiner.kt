...
class HangulCombiner : Combiner {
    ...
    override fun processEvent(previousEvents: ArrayList<Event>?, event: Event?): Event? {
        Log.d(
            "HangulCombiner", "ENTER  processEvent: mKeyCode=${event?.mKeyCode} " +
        ...
        if (Character.isWhitespace(event.mCodePoint)) {
            ...
            EventBus.getDefault()
                .post(HangulCommitEvent(HangulCommitEvent.TYPE_END, text.toString()))
            reset()
...
//            if(!event.isCombining || jamo is HangulJamo.NonHangul) {
            if (jamo is HangulJamo.NonHangul) {
                ...
                if (composingWord.isNotEmpty()) {
                    val commitEvt = createEventChainFromSequence(composingWord.toString(), event)
                    composingWord.setLength(0)
                    // Non-Hangul 문자는 그대로 다음 이벤트로 이어서 처리
                    return commitEvt
                }
                ...
                when (jamo) {
                    is HangulJamo.Consonant -> {
                        ...
                        if (currentSyllable.initial != null && currentSyllable.medial != null) {
                            if (currentSyllable.final == null) {
                                val combination =
                                    COMBINATION_TABLE_DUBEOLSIK[currentSyllable.initial.codePoint to (initial?.codePoint
                                        ?: -1)]
                                if (combination != null) {
                                    history += currentSyllable.copy(
                                        initial = HangulJamo.Initial(
                                            combination
                                        )
                                    )
                                } else {
                                    if (final != null) history += currentSyllable.copy(final = final)
                                        ...
                                        // ── 직전 음절 완성 : 바로 커밋하고 buffer 비우기 ────────────────────────
                                        if (composingWord.isNotEmpty()) {
                                            val commitEvt = createEventChainFromSequence(
                                                composingWord.toString() + currentSyllable.string,
                                                event
                                            )
                                            composingWord.setLength(0)
                                            history.clear()
                                ...
                                val pair =
                                    currentSyllable.final.codePoint to (final?.codePoint ?: -1)
                                val combination = COMBINATION_TABLE_DUBEOLSIK[pair]
                                if (combination != null) {
                                    history += currentSyllable.copy(
                                        final = HangulJamo.Final(
                                            combination,
                                    ...
                                    if (composingWord.isNotEmpty()) {
                                        val commitEvt = createEventChainFromSequence(
                                            composingWord.toString(),
                                            event
                                        )
                                        composingWord.setLength(0)       // buffer 비우기
                                        // 커밋-이벤트 뒤에 이번 키 입력을 이어 붙여 재처리
                                        return commitEvt
                                    }
                    ...
                    is HangulJamo.Vowel -> {
                        ...
                        if (currentSyllable.final == null) {
                            if (currentSyllable.medial != null) {
                                val combination =
                                    COMBINATION_TABLE_DUBEOLSIK[currentSyllable.medial.codePoint to (medial?.codePoint
                                        ?: -1)]
                                if (combination != null) {
                                    history += currentSyllable.copy(
                                        medial = HangulJamo.Medial(
                                            combination
                                        )
                                    )
                                } else {
                    ...
                    is HangulJamo.Initial -> {
                        if (currentSyllable.initial != null) {
                            val combination =
                                COMBINATION_TABLE_SEBEOLSIK[currentSyllable.initial.codePoint to jamo.codePoint]
                    ...
                    is HangulJamo.Medial -> {
                        if (currentSyllable.medial != null) {
                            val combination =
                                COMBINATION_TABLE_SEBEOLSIK[currentSyllable.medial.codePoint to jamo.codePoint]
                            if (combination != null) {
                                history += currentSyllable.copy(
                                    medial = HangulJamo.Medial(
                                        combination
                    ...
                    is HangulJamo.Final -> {
                        if (currentSyllable.final != null) {
                            val combination =
                                COMBINATION_TABLE_SEBEOLSIK[currentSyllable.final.codePoint to jamo.codePoint]
                            if (combination != null) {
            ...
            if (jamo is HangulJamo.Consonant || jamo is HangulJamo.Vowel ||
                ...
                EventBus.getDefault()
                    .post(HangulCommitEvent(HangulCommitEvent.TYPE_SYLLABLE, text.toString()))
            }
        }
        Log.d(
            "HangulCombiner",
            "EXIT   processEvent: combiningStateFeedback=\"$combiningStateFeedback\""
        )
    ...
    sealed class HangulJamo {
        ...
        data class Initial(override val codePoint: Int) : HangulJamo() {
            ...
            fun toConsonant(): Consonant? {
                val codePoint =
                    COMPAT_CONSONANTS.getOrNull(CONVERT_INITIALS.indexOf(codePoint.toChar()))
                        ?: return null
        ...
        data class Medial(override val codePoint: Int) : HangulJamo() {
            ...
            fun toVowel(): Vowel? {
                val codePoint = COMPAT_VOWELS.getOrNull(CONVERT_MEDIALS.indexOf(codePoint.toChar()))
                    ?: return null
                return Vowel(codePoint.toInt())
            }
        }

        data class Final(override val codePoint: Int, val combinationPair: Pair<Int, Int>? = null) :
            HangulJamo() {
            override val modern: Boolean get() = codePoint in 0x11a8..0x11c2
            val ordinal: Int get() = codePoint - 0x11a7
            fun toConsonant(): Consonant? {
                val codePoint =
                    COMPAT_CONSONANTS.getOrNull(CONVERT_FINALS.indexOf(codePoint.toChar()))
                        ?: return null
                if (codePoint.toInt() == 0) return null
                return Consonant(codePoint.toInt())
            }
        ...
        data class Consonant(override val codePoint: Int) : HangulJamo() {
            ...
            fun toInitial(): Initial? {
                val codePoint =
                    CONVERT_INITIALS.getOrNull(COMPAT_CONSONANTS.indexOf(codePoint.toChar()))
                        ?: return null
                if (codePoint.toInt() == 0) return null
            ...
            fun toFinal(): Final? {
                val codePoint =
                    CONVERT_FINALS.getOrNull(COMPAT_CONSONANTS.indexOf(codePoint.toChar()))
                        ?: return null
                if (codePoint.toInt() == 0) return null
        ...
        data class Vowel(override val codePoint: Int) : HangulJamo() {
            ...
            val ordinal: Int get() = codePoint - 0x314f
            fun toMedial(): Medial? {
                val codePoint = CONVERT_MEDIALS.getOrNull(COMPAT_VOWELS.indexOf(codePoint.toChar()))
                    ?: return null
                if (codePoint.toInt() == 0) return null
        ...
        companion object {
            ...
            const val CONVERT_INITIALS =
                "ᄀᄁ\u0000ᄂ\u0000\u0000ᄃᄄᄅ\u0000\u0000\u0000\u0000\u0000\u0000\u0000ᄆᄇᄈ\u0000ᄉᄊᄋᄌᄍᄎᄏᄐᄑᄒ"
    ...
    data class HangulSyllable(
        ...
        val combinable: Boolean
            get() = (initial?.modern ?: false) && (medial?.modern ?: false) && (final?.modern
                ?: true)
        val combined: String
            get() = (0xac00 + (initial?.ordinal ?: 0) * 21 * 28
                    + (medial?.ordinal ?: 0) * 28
                    + (final?.ordinal ?: 0)).toChar().toString()
        val uncombined: String
            get() = (initial?.string ?: "") + (medial?.string ?: "") + (final?.string ?: "")
        val uncombinedCompat: String
            get() = (initial?.toConsonant()?.string ?: "") +
