...
// that result in input like letters or space.
    ...
    /** 이번 이벤트로 완성형 한글 음절이 하나 확정됐는지 */
    val isHangulSyllableCompleted: Boolean
        get() = 0 != FLAG_HANGUL_SYLLABLE_COMPLETED and mFlags
...
// or dead-abovering.
        ...
        /* ---------- 새로 추가 ---------- */
        /** Hangul Combiner가 방금 “완성형 음절”을 `textToCommit` 으로 돌려줬음을 알림 */
        const val FLAG_HANGUL_SYLLABLE_COMPLETED = 0x10
        /* -------------------------------- */
        ...
        fun createCombiningEvent(source: Event): Event {
            return Event(
                source.mEventType, source.mText, source.mCodePoint, source.mKeyCode,
                source.mX, source.mY, source.mSuggestedWordInfo,
                source.mFlags or FLAG_COMBINING, source.mNextEvent
            )
        }

        /** 기존 이벤트에 플래그를 하나 더 달아 복사본을 돌려줌 */
        fun addFlags(source: Event, extraFlags: Int): Event =
            Event(
                source.mEventType, source.mText, source.mCodePoint, source.mKeyCode,
                source.mX, source.mY, source.mSuggestedWordInfo,
                source.mFlags or extraFlags, source.mNextEvent
            )
        /* -------------------------------- */
